...
class HtmlTagBuilderTest < Minitest::Test

  include ::HtmlTagBuilder::Helper

  def test_that_it_has_a_version_number
    refute_nil ::HtmlTagBuilder::VERSION
  end
  ...
  def test_tag_builder_options_converts_boolean_option
    assert_equal '<p disabled="disabled" itemscope="itemscope" multiple="multiple" readonly="readonly" allowfullscreen="allowfullscreen" seamless="seamless" typemustmatch="typemustmatch" sortable="sortable" default="default" inert="inert" truespeed="truespeed"></p>',
  ...
  def test_tag_builder_nested
    ...
    assert_equal "<div id=\"header\"><span>hello</span></div>",
                 tag.div(id: "header") { |b| b.tag.span "hello" }
    assert_equal "<div id=\"header\"><div class=\"world\"><span>hello</span></div></div>",
                 tag.div(id: "header") { |b| b.tag.div(class: "world") { tag.span "hello" } }
  end
  ...
  def test_tag_builder_with_block_in_erb_containing_nested_tags
    buffer = render_erb("<%= tag.p do |b| %><% 1 %><%= b.tag.br %>bar<%= 'foo' %><% b.text 'blah' %><% end %>")
